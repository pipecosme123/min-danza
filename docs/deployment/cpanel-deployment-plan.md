...
### 9.4 `actions/upload-artifact@v4` excluye dotfiles por defecto — se comía el `.htaccess` en cada deploy (encontrado en producción el 2026-08-29)

El primer deploy manual (§8, 2026-08-11) subió `client/dist/.htaccess` por SSH directo y el checklist de §7 confirmó en su momento que refrescar `/admin/eventos` no daba 404. Una vez el pipeline automatizado quedó funcionando (§9, 2026-08-22), cada deploy automático volvió a romperlo sin que nadie lo notara hasta que un usuario reportó el 404 real: el job `test-frontend` sube `client/dist` completo con `actions/upload-artifact@v4`, pero esa acción **excluye archivos ocultos por defecto** — el artifact resultante nunca contuvo `.htaccess` (confirmado descargándolo con la API de Actions y listando su contenido: 6 archivos, sin `.htaccess`). El job `deploy` baja ese artifact y lo sincroniza al servidor con `rsync -az --delete`, así que el `.htaccess` que sí había quedado del deploy manual inicial se borró en el primer deploy automatizado y siguió sin subirse en todos los siguientes — sin fallar el pipeline ni el smoke test (que solo verifica `/health` del backend), quedó completamente silencioso. Diagnosticado sin SSH: reproducido primero en el navegador (`/admin/eventos` y `/admin/login` devuelven el 404 nativo de LiteSpeed, no el de React Router, señal de que ni siquiera se evalúa el rewrite) y confirmado bajando el artifact real del último run exitoso vía la API de Actions (mismo token OAuth cacheado de §9.3). Fix: agregar `include-hidden-files: true` al paso `actions/upload-artifact@v4` de `test-frontend` en `deploy.yml` — input soportado desde v4.4 específicamente para este caso.
